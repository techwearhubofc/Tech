    ...
    Themes = {
     	NeonRed = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(30, 5, 5)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(55, 10, 10)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(30, 5, 5))
		 }),
     	["Color Hub 2"] = Color3.fromRGB(45, 8, 8), 
     	["Color Toggle"] = Color3.fromRGB(3, 3, 3), -- bola do toggle
     	["Color Stroke"] = Color3.fromRGB(255, 40, 40), -- vermelho neon
     	["Color Theme"] = Color3.fromRGB(255, 0, 60), -- vermelho neon principal
     	["Color Text"] = Color3.fromRGB(240, 240, 240),
     	["Color Dark Text"] = Color3.fromRGB(255, 120, 120)
		},
...
local Settings = techlib.Settings
local Flags = techlib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(techlib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then techlib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then techlib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then techlib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, techlib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(techlib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = techlib.Themes[techlib.Save.Theme]

local function AddEle(Name, Func)
	techlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = techlib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function techlib:GetIcon(index)
...
function techlib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	techlib.Save.Theme = NewTheme
	SaveJson("redz library V5.json", techlib.Save)
	Theme = techlib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(techlib.Instances, function(_,Val)
...
function techlib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function techlib:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(techlib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, techlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			techlib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("redz library V5.json", techlib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		techlib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("redz library V5.json", techlib.Save)
	end)
	...
	function Window:CloseBtn()
		local Dialog = Window:Dialog({
			Title = "Close",
			Text = "Tem Certeza que deseja fechar o Techwear Hub? Pode acabar interferindo em funções importantes!",
			Options = {
				{"Confirmar", function()
					ScreenGui:Destroy()
				end},
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			techlib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(techlib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(techlib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(techlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(techlib.Options, {type = "Section", Name = SectionName, func = Section})
